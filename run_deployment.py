...
def main(config: str, min_accuracy:float):
    ...
    if deploy:
        continuous_deployment_pipeline(
            ...
            min_accuracy=min_accuracy, workers=3, timeout=60
        )
    if predict:
        inference_pipeline(
    ...
    if existing_services:
        ...
        if service.is_running:
            ...
            print(
                f"The MLFlow prediction server is in an failed state:\n"
                f" Last state: '{service.status.state.value}'\n"
                f" Last error: '{service.status.last_error}'"
                )
    else:
        print(
            "No MLflow prediction server is currently running. The deployement "
